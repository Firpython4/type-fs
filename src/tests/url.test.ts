...
test("urlTest1: Should parse a valid URL from a .url file", async () => {
  const fileMocker = createFileMocker(
    toPath("test-resources/url/urlTest1"),
...
test("urlTest2: Should fail for files with an invalid extension", async () => {
  const fileMocker = createFileMocker(
    toPath("test-resources/url/urlTest2"),
...
test("urlTest3: Should fail for invalid URL content", async () => {
  const fileMocker = createFileMocker(
    toPath("test-resources/url/urlTest3"),
...
test("urlTest4: Should fail for missing file", async () => {
  ...
  const result = await urlSchema.parse(
    toPath("test-resources/url/missing.url"),
  );
...
test("urlTest5: Should correctly parse URL inside an object schema", async () => {
  const fileMocker = createFileMocker(
    toPath("test-resources/url/urlTest5"),
...
test("getUrl: Should read URL content from file", async () => {
  const fileMocker = createFileMocker(
    toPath("test-resources/url/getUrlTest"),
  ).createFile(toPath("test.url"), "https://www.example.com");
